...
export const CreateProjectModal: FC<CreateProjectModalProps> = ({
  ...
  const getCurrentColorScheme = () => {
    return (
      PRESET_COLORS.find((color) => color.bg === formData.color) ||
      PRESET_COLORS[0]
    );
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 backdrop-blur-sm z-50">
      <div
        className={`bg-gray-900 rounded-3xl p-10 shadow-2xl w-full max-w-2xl border transition-all duration-300 ${colorScheme.border}`}
      >
        <div className="flex justify-between items-center mb-6">
          <h2
            className={`text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r ${colorScheme.from} ${colorScheme.to}`}
          >
            Create New Project
          </h2>
          <button
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 bg-gray-800 rounded-xl border border-gray-700 focus:border-${
                formData.color.split("-")[1]
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              <input
                ...
                className={`flex-1 px-4 py-2 bg-gray-800 rounded-xl border border-gray-700 focus:border-${
                  formData.color.split("-")[1]
                }-500 focus:ring-2 focus:ring-${
                  formData.color.split("-")[1]
                }-500 outline-none transition-all`}
